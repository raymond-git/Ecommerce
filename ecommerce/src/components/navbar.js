    ...
    return (
        ...
        <div className="flex justify-between py-6 p-14 bg-black"> 
            <div className='flex justify-start'>
                ...
                <h1 className='text-white text-2xl ml-4 font-sans mt-1'>CloudCart</h1>
            </div>
            <div className="relative">
                <Link to={`/cart`}><BsCart3 className="text-white h-10 w-8 cursor-pointer" /></Link>
                <div className="absolute top-0 right-0 transform translate-x-1/2 -translate-y-1/2 bg-red-500 w-6 h-6 rounded-full flex items-center justify-center text-white text-sm">{cartCount}</div>
            </div>
        </div>
    )
}
